...
| V-4 | B2 experiment-system code (`cvce/research_engine/{technique_registry,experiment_matrix,constraint_trace,synthesis}.py`) — internal research infra, failed independent review 3× (last 66/100), landed live via commit `7cc0acc`. Zero live exposure confirmed (not referenced by `server.py`, gated off by default via `CVCE_RESEARCH_MODE_ENABLED`). | High | 2026-07-18 | ☑ **Resolved by owner (2026-08-11): remediate via the same cycle used for B3.** Sequenced after V-1/V-7 (Vercel migration) per strategy — not urgent given zero live exposure, but committed to. |
| V-5 | `knowledge-graph/graphify-out/memory-state/` (1.8MB: `runtime.json` + `node_embeddings.npz`) is untracked and not gitignored — looks like generated runtime cache from the new self-evolving memory system, not source. Blocks a clean `git add` of anything else in that tree until classified. | Medium | 2026-08-11 | ◐ **Needs you** — commit as versioned state (matches this repo's existing policy of committing `graph.json`), or gitignore as regenerable cache? |
| V-6 | `supabase/migrations/20260721232500_enable_rls_guest_charts.sql` is an uncommitted, well-documented **security fix** — enables RLS on `guest_charts`, which Supabase's advisor flagged 2026-07-21 as publicly readable/writable/deletable via the anon key. Live-apply status unverified. | Medium | 2026-08-11 | ☐ Commit regardless of live-apply status. |
...
| V-12 | New, opened from V-3's resolution: `vedic_knowledge` package is designed to be shared by VedicAstro *and* panchanga_muhurtha, which is in tension with the "zero influence onto Muhurtha" freeze principle if Muhurtha ever adopts it. Not yet confirmed whether panchanga_muhurtha actually imports it today. | Medium | 2026-08-11 | ☐ Needs a check on the panchanga_muhurtha side before that package is treated as safe to keep sharing. |
...
| V-14 | 2 chapters of `Phaladeepika_Mantreswara_1961` never embedded — DeepSeek API returned `402 Insufficient Balance` mid-run (slices 30/31). | `ingest-logs/status-report.log` | Medium | 2026-08-11 | ☐ Top up that API's balance and re-run, if that book's embeddings need to be complete. |
| V-15 | 391 dormant git worktrees, `.kilo/worktrees/ensure-active-agents-*` (1–551), none active (confirmed via `ps`/`launchctl`). Disk bloat only, no functional harm. | Owner-reported | Low | 2026-08-11 | ☐ `git worktree prune` or manual cleanup whenever convenient. |
| V-16 | `com.vedicastro.ingest` launchd daemon has a crash-loop history — dozens of "sync-watch died — restarting" within ~20min (2026-06-29). Currently idle/dormant, underlying flakiness in `ingest-pipeline-daemon.sh`'s sync-watch was never actually fixed, just outran the issue. | `launchd.out.log` | Low | 2026-08-11 | ☐ Worth a look only before re-running ingestion — may recur. |
| V-17 | `graph-deepseek.json` has fewer nodes (23,267) than `graph.json`/`graph-core-jyotisha.json` (26,722). Not confirmed as a bug — could be an earlier/different extraction pass. | Status report, flag-only | Low | 2026-08-11 | ☐ Not investigated, out of scope for the reporting session. |
| V-8 | Transit Context (portal+CVCE) previously failed independent review at 68/100 (`RELEASE_HANDOFF.md`, 2026-07-14). Partially touched 2026-07-18 (blank-coordinate/timezone/stale-year fixes, commit `e2c1337`) but that was never run through a full remediation→re-gate cycle — the 68/100 verdict itself was never overturned. | Medium | 2026-07-14 (carried over) | ☐ Re-gate needed before this can be considered resolved. |
| V-9 | `docs/prediction-engine-strategy/RELEASE_HANDOFF.md` still says B3's "Independent re-gate is pending" — it isn't; B3 passed at 97/100 on 2026-07-18. Stale doc, will mislead the next reader. | Low | 2026-08-11 | ☐ |
| V-10 | `cvce/graph_rag/_original_backup/`, `cvce/knowledge_engine/_original_backup/` — untracked pre-refactor safety copies, harmless but should be deleted once V-1 is fixed and the shim diff is verified+committed. | Low | 2026-08-11 | ☐ |
