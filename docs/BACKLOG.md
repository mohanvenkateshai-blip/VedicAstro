...
**Owner directive 2026-08-22T21:28:16+0100:** Retire the sunset Fly CVCE deployment and
stop its unattended health-monitor schedule. The GitHub workflow schedule is now disabled
while manual dispatch remains available for the one-week Vercel observation window. Fly
...
|---|---|---|---|---|
| V-18 | **Real bug found during V-7's cutover verification, not before.** `/chart` on the freshly-redeployed Vercel CVCE returned `500: chart geometry failed: AttributeError: 'dict' object has no attribute 'index'`. Root cause: upstream `PyJHora==4.8.7`'s own `drik.planetary_positions()` calls `drik.planet_list.index(planet)`, but `drik.planet_list` is a dict, not a list — a genuine bug in the third-party package, reproduced from a clean `pip install PyJHora==4.8.7` in a fresh Python 3.12 venv (matching Vercel's build exactly). It never showed up locally or on Fly because those environments' installed copies of the "same" 4.8.7 have this exact line already fixed by hand at some point in `site-packages` — an undocumented, untracked local patch nobody recorded anywhere (not in `requirements.txt`, not in a patch script, not in project code). Every fresh install of this dependency (this Vercel deploy, a Fly rebuild without cache, a new dev machine, CI) would have hit this and silently broken `/chart`, `/natal`, `/report/facts`, `/predict` — effectively the whole product. | **Critical** | 2026-08-11 | ☑ **Fixed properly (2026-08-11):** added a monkeypatch in `cvce/app/ephem.py` (the one file allowed to import PyJHora) that replaces `drik.planetary_positions` with a corrected implementation at import time — survives any fresh install regardless of platform, until upstream fixes it. Verified: reproduces the crash and confirms the fix in a clean Python 3.12 venv matching Vercel's exact build; full CVCE suite green (exit 0); golden reference chart (Lagna Libra/Swati pada 4) correct on the redeployed Vercel CVCE, stable across 3 live authenticated requests. |
| V-1 | Uncommitted shim/de-dup diff activated `vedic_knowledge` as the primary knowledge-engine path — but `vedic_knowledge/graph/enhancer.py` imported `vedic_engine.synthesis.transit_analyzer`, and `vedic_engine` is VedicAstro-local code, not part of the "shared, portable" package. Proven **non-deterministic**: isolated test runs hard-crashed (`KeyError` inside `importlib._bootstrap_external`, not a catchable `ImportError`), full-suite runs succeeded *silently with different behavior*. **Independently confirmed via a separate panchanga_muhurtha-side investigation** of a real production incident — same `KeyError: 'vedic_engine'`. Also blocked the `query_research_nodes()` egress-caching fix bundled in the same diff (see V-13) from shipping alone. | **Critical** | 2026-08-11 (this review) | ☑ **Fixed, verified, committed (`c779525`), and pushed to `origin/main`.** Full detail below at "V-1 — fixed 2026-08-11". *(Corrected 2026-08-11: this row previously still said ☐/blocking — stale, left over from before the commit; the fix has been live on `origin/main` since early in this session.)* |
...
| V-7 | Fly→Vercel migration. | Medium | 2026-08-11 | ☑ **Fully cut over and verified (2026-08-11).** Owner-approved cause: Muhurtha's Fly free tier was unintentionally billed and froze, no budget for Fly. Sequence completed in order per owner instruction: (1) pushed 5 pending local commits to `origin/main`; (2) generated and synced a new shared `CVCE_SERVICE_TOKEN` across both the `vedicastro` (portal) and `vedicastro-cvce-vercel` (CVCE) Vercel projects' Production env, replacing two previously-mismatched values; (3) found and fixed a real upstream bug during verification, not before — see **V-18**; (4) set `CVCE_BASE_URL=https://vedicastro-cvce-vercel.vercel.app` on the portal's Production env and redeployed; (5) verified with a real end-to-end request through the actual production path — `https://portal-omega-two-10.vercel.app/api/cvce/positions` → the redeployed Vercel CVCE — returned HTTP 200 with the correct golden reference chart (Lagna Libra, Swati nakshatra, pada 4). Fly remains up as a fallback; not decommissioned this pass. |
...
| V-13 | **Real incident, already happened.** `query_research_nodes()` → `_enumerate_current_research_nodes()` (`cvce/knowledge_engine/engine.py`) did a full unfiltered Supabase `graph_nodes` table scan with no caching. Traced via Supabase Edge Logs (panchanga_muhurtha-side investigation): dozens of overlapping full-table walks in 72s, ~5.4GB egress this cycle, tripped the org-wide Supabase quota, broke Muhurtha's admin dashboard. This is *why* panchanga_muhurtha was stalled and this review happened. | **Critical** | 2026-08-11 (reported by owner, from Muhurtha-side investigation) | ☑ **Shipped** — the TTL + row-count-freshness cache landed in the same commit as V-1 (`c779525`), now on `origin/main` and live on the Vercel CVCE deployment per V-7. **Update 2026-08-22 — durable fix built (not yet cut over):** the TTL tweak only reduces cache-miss *rate*; the root fix is taking graph reads off Supabase's live path entirely, per owner-authored `docs/graph-sqlite-migration-playbook_1.md`. Built: `cvce/scripts/build_graph_db.py` (streaming JSON→SQLite bake, adapted to the real schema — node-link `{nodes, links, hyperedges}`, not the playbook's assumed `{nodes, edges}`; verified against the live 26,722-node/38,881-link/1,773-hyperedge corpus, 18.6MB output, 0.6s build, dangling-link counts match exactly: 58 src/157 dst); `cvce/knowledge_engine/store/sqlite_store.py` (`SQLiteKnowledgeStore`, same `KnowledgeStore` interface/field shapes as `SupabaseKnowledgeStore` — callers unchanged); `KnowledgeEngine.with_sqlite()`; `GRAPH_SOURCE=sqlite\|supabase` flag wired into `get_knowledge_engine()` in **both** copies of `integration.py` (`cvce/vedic_knowledge/` and the vendored `vedic-knowledge/vedic_knowledge/`), **defaults to `supabase`** (current behavior unchanged) until parity is verified; multi-stage `Dockerfile` (Fly fallback, kept current though not the live path); two GitHub Actions workflows (`publish-graph-snapshot.yml`, `build-and-deploy-cvce.yml` — the latter deploys via Vercel **Deploy Hook**, not the CLI, because token-based Vercel CLI auth is documented-broken for this account, see panchanga_muhurtha `ci.yml`); `scripts/check_graph_parity.py` (spot-check tool for migration checklist §7); 9 new unit tests, all passing, full existing CVCE suite re-run clean (no regressions). **Deliberate scope decisions:** `graph_rag/graph.json` stays in the served artifact (not stripped) — `engine.py`'s `_attach_graph_compat()` always builds the legacy `GraphRAG` accessor straight from that file regardless of which `KnowledgeStore` backend is active, so removing it would silently break that existing fallback; `hyperedges` are baked into `graph.db` but not yet exposed via any `KnowledgeStore` method (neither backend serves them through that interface today). **Not done yet:** the two GH Actions secrets this needs (`DEPLOY_HOOK_CVCE`, plus confirming `GITHUB_TOKEN` has `contents: write` in that repo's Actions settings) are a one-time manual dashboard step, not something committable; parity check has never actually run (needs `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` in whatever environment runs it — not present in this local checkout); `GRAPH_SOURCE=sqlite` has not been flipped anywhere live. **Next action:** owner wires the Deploy Hook secret, someone runs `check_graph_parity.py` with real Supabase creds, then flip `GRAPH_SOURCE=sqlite` on the Vercel CVCE project's env and watch Supabase egress on the graph tables go to ~0. |
...
| # | Issue | Resolution |
|---|---|---|
| — | Whether any of the Aug 1–4 work is live in production | At review time (start of this session): verified no — Fly still on the 2026-07-18 image, portal still pointed at Fly by default. **Superseded 2026-08-11 by V-7**: portal's production `CVCE_BASE_URL` now points at the Vercel CVCE deployment, which includes the Aug 1–11 fixes (V-1, V-13, V-18). |
...
**Done:** committed in the two clean groups from the original review plan — shim/de-dup +
`vedic_engine` decoupling + test fixes as `c779525`, `supabase` RLS migration (V-6) separately —
