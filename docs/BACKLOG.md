...
**Complete issue history:** `docs/handoff/SUPABASE_EGRESS_RETIREMENT_2026-08-22.md` records
the incident, interim cache mitigation, SQLite cutover, Admin/Learn removals, background
monitor investigation, Fly scale-to-zero execution, exact commits/deployments, and the
remaining one-week observation gate.
...
| V-13 | **Real Supabase egress incident.** `query_research_nodes()` → `_enumerate_current_research_nodes()` performed unfiltered `graph_nodes` scans; the panchanga_muhurtha-side Edge Log investigation found dozens of overlapping walks in roughly 72 seconds, about 5.4 GB egress, and an org-wide quota trip that broke Muhurtha's Admin dashboard. | **Critical** | 2026-08-11 | ◐ **Mitigated and cut over; observation remains.** `c779525` shipped the TTL/row-count cache; `2f0c656` reduced TTL to 60 seconds as an interim measure. The durable SQLite path landed in `ff39ace`, was bundled and diagnosed via `161c4be`, and production `GRAPH_SOURCE=sqlite` was verified by `dpl_i1QKUoMbvPvqKofybAvxu4hcWkXB` with 26,722 nodes and 38,881 links. The detailed chronology, exact checks, remaining Supabase consumers, 21 Aug usage evidence, health-monitor investigation, and Fly retirement are in `docs/handoff/SUPABASE_EGRESS_RETIREMENT_2026-08-22.md`. It is not safe to claim all Supabase traffic is zero until the one-week observation and log review pass. |
...
| V-7 | Fly→Vercel migration. | Medium | 2026-08-11 | ☑ **Production cutover verified 2026-08-11; fallback retired 2026-08-22.** Owner-approved cause: Muhurtha's Fly free tier was unintentionally billed and froze, with no budget for Fly. Vercel became the live portal/CVCE path, production `/api/cvce/positions` returned the golden chart, and `GRAPH_SOURCE=sqlite` was later verified live. The former Fly fallback was scaled to zero under V-21; the app is preserved but no machine runs. |
...
| V-13/B-56 | Use baked SQLite for normal production graph reads; retain Supabase only for explicitly preserved consumers until observation proves the egress reduction. | Owner | 2026-08-22 |
| V-19 | Remove Learn from the portal; do not build or migrate its reader. | Owner | 2026-08-22 |
| V-20 | Remove the diagnostic Admin corpus explorer and direct portal graph queries; do not delete shared Supabase services. | Owner | 2026-08-22 |
| V-21 | Make Vercel the sole production path; disable the scheduled Fly monitor, scale Fly to zero, retain manual rollback monitoring for one week. | Owner | 2026-08-22 |
