  ...
  return (
    <div>
      ...
      >
        ...
        {style && LeapIcon && (
          <span
            className={clsx("grid h-7 w-7 shrink-0 place-items-center rounded-lg", style.bgClass)}
          >
            <LeapIcon className={clsx("h-4 w-4", style.colorClass)} />
          </span>
        )}
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {leap && style && (
              <button
                ...
                className={clsx(
                  "flex items-center gap-1.5 rounded-full px-2 py-0.5 text-xs font-semibold tracking-tight",
                  style.colorClass,
                  style.bgClass,
                )}
              >
                ...
                {leap.strength && (
                  <span
                    className={clsx("h-1.5 w-1.5 rounded-full", strengthStyle(leap.strength).dotClass)}
                    title={strengthStyle(leap.strength).label}
                  />
                )}
              </button>
            )}
          ...
          <div className="h-1 rounded-full bg-hairline/40 mt-1.5 overflow-hidden">
            <motion.div
              className={clsx("h-full rounded-full", running ? "bg-accent" : style ? style.solidClass : "bg-text-muted/40")}
              initial={{ width: 0 }}
