...
def preprocess_image(
    img: Image.Image, contrast: float = 2.0, sharpness: float = 1.0, denoise: bool = False
) -> Image.Image:
...
def ocr_single_page(
    ...
    images = convert_from_path(
        pdf_path, dpi=300, first_page=page_num + 1, last_page=page_num + 1, thread_count=1
    )
...
def process_pdf(
    ...
    try:
        ...
        print(
            f"Warning: Could not open PDF with pdfplumber: {e}. Falling back to pypdf.",
            file=sys.stderr,
        )
    ...
    if not missing_pages:
        ...
        print(
            f"Queueing {len(missing_pages)} pages for OCR processing using {max_workers} worker threads/processes..."
        )
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        if res.get("success", False):
                            ...
                            fail_count += 1
                            print(
                                f"\nPage {page_num_1indexed} failed: {res.get('error')}",
                                file=sys.stderr,
                            )
                    except Exception as e:
                        fail_count += 1
                        print(
                            f"\nPage {page_num_1indexed} generated an exception: {e}",
                            file=sys.stderr,
                        )
                    pbar.update(1)
...
def main():
    ...
    parser.add_argument(
        "--input-dir",
        type=str,
        default="/Users/ganesha/Projects/04-UX-Practice/Panchang/Gyan",
        help="Directory containing source PDFs",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="/Users/ganesha/Projects/04-UX-Practice/Panchang/Gyan/extracted_markdown",
    ...
    parser.add_argument(
        "--contrast", type=float, default=2.0, help="Contrast enhancement factor (default: 2.0)"
    )
    parser.add_argument(
        "--sharpness", type=float, default=1.0, help="Sharpness enhancement factor (default: 1.0)"
    )
    parser.add_argument(
        "--denoise", action="store_true", help="Apply 3x3 median filter to denoise scans"
    )
    ...
    try:
        ...
        print(
            "Please check your network connection or manually copy traineddata files to the tessdata folder.",
            file=sys.stderr,
        )
        sys.exit(1)
