...
## Session checkpoint — 2026-08-11 (backlog review after a 3.5-week handoff gap; V-1/V-13 production incident fixed; Vercel migration in progress)

- **Why this checkpoint exists:** ~3.5 weeks of work (2026-08-01 → 2026-08-11) landed with zero handoff-doc updates — this section is that catch-up. Full tracking: `docs/BACKLOG.md` (V-1 through V-17, criticality + evidence + resolution for each).
...
- **B-16 Phase 2:** `graph_rag` + `knowledge_engine` extracted into a shared, portable pip package (`vedic_knowledge`, sibling repo `VedicAstro/vedic-knowledge/`) meant for both VedicAstro and panchanga_muhurtha to consume. VedicAstro's own `cvce/graph_rag/` and `cvce/knowledge_engine/` reduced to thin re-export shims over it. panchanga_muhurtha bridges to it via `api/vedic_knowledge_bridge.py` — HTTP call to VedicAstro's deployed CVCE in production; an in-process pip-import path exists but is local-dev-only and not in their `requirements.txt` (checked 2026-08-11, no production coupling risk — see V-12).
- **Self-evolving memory system:** new `auto_mapper.py`, `schema_mutator.py`, `session_memory.py` under `cvce/knowledge_engine/`, an ingest-watcher, and `/memory/*` API endpoints — lets the knowledge graph propose/track structural mutations from ingest activity instead of requiring a manual promotion pass every time.
- **CVCE → Vercel vendoring (`04d2ffd`):** because Vercel Functions only upload the linked project root (not sibling directories), the shared `vedic_knowledge` package is vendored into `cvce/vedic_knowledge/` (kept in sync with the sibling source) so a Vercel-hosted CVCE can import it without a monorepo. New Vercel project `vedicastro-cvce-vercel` (stable alias `vedicastro-cvce-vercel.vercel.app`) created as a parallel deployment target alongside the existing Fly.io one.
- **2026-08-11 review found and fixed a real, already-live production incident:** `vedic_knowledge/graph/enhancer.py` imported VedicAstro-local `vedic_engine` from inside the "portable" shared package — a non-deterministic bug (uncatchable `KeyError` deep in `importlib`, not a normal `ImportError`) that depended on which test/module happened to import `vedic_engine` first. This wasn't just theoretical: it was independently traced from the *panchanga_muhurtha* side to a real Supabase org-wide egress-quota trip (~5.4GB in 72s, dozens of overlapping full-table scans from an uncached `_enumerate_current_research_nodes()`) that broke that app's admin dashboard. **Fixed** via dependency injection — `PredictionEnhancer` now takes `transit_analyzer` as a constructor param instead of importing it; the one real caller (`cvce/app/report_facts.py`) injects `TransitImpactAnalyzer()` locally. The egress fix (TTL + row-count-freshness cache) shipped in the same commit since it couldn't be fixed independently (its own import hit the same crash chain). 340/340 CVCE tests pass (was 336/340), stable across repeated full-suite and isolated-file runs. Full incident trail: `docs/BACKLOG.md` V-1 and V-13.
- **V-7, Fly→Vercel migration (owner-approved 2026-08-11, root cause: Muhurtha's Fly free tier was unintentionally billed and froze, no budget for Fly):** CVCE redeployed to `vedicastro-cvce-vercel.vercel.app` with the V-1/V-13 fixes — `/health` verified stable (200 × 5). **Not yet cut over** — found the `CVCE_SERVICE_TOKEN` shared secret differs between the portal's Vercel project and the CVCE-on-Vercel project (never synced), which would 401 every authenticated call if the portal were pointed at it right now. Needs the owner to either sync that token on both Vercel projects or authorize the sandbox-blocked `vercel env` write, then flip `portal`'s production `CVCE_BASE_URL` (currently unset, defaults to Fly).
- **8 local commits are ahead of `origin/main`, not yet pushed** — `git push` was blocked by the sandbox's safety classifier as a shared-state action; needs the owner to push or explicitly authorize it.
- **Other backlog items closed 2026-08-11:** stale `RELEASE_HANDOFF.md` B3 text (said re-gate "pending", actually passed 97/100 on 2026-07-18); 456/458 dormant `.kilo/worktrees/*` pruned; `graph-deepseek.json`'s lower node count confirmed as a separate diagnostic-only extraction, not a bug.
- **Still open:** V-4 (B2 experiment-system remediation, `cvce/research_engine/{technique_registry,experiment_matrix,constraint_trace,synthesis}.py`, last scored 66/100) and V-8 (Transit Context remediation, last scored 68/100) both need a fresh independent review cycle — the original per-finding detail wasn't preserved anywhere in the repo, only the verdict. V-11 (Life-Event Prediction engine, see the 2026-07-17 checkpoint below) still has zero implementation.
