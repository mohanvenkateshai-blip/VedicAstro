...
## Session checkpoint — 2026-08-11 (backlog review after a 3.5-week handoff gap — full remediation cycle complete, V-7 Vercel cutover live, V-11 gated)

**End-of-session state: 17 of 18 tracked backlog items closed. Working tree clean, everything
pushed to `origin/main`.** Full detail, evidence, and verification trail for every item:
`docs/BACKLOG.md` (V-1 through V-18).

- **Why this checkpoint exists:** ~3.5 weeks of work (2026-08-01 → 2026-08-11) landed with zero handoff-doc updates — this section is that catch-up, closed out in the same session it was opened.
- **B-16 Phase 1 (2026-08-01-ish):** panchanga_muhurtha's activity-finder logic ported to Python inside CVCE — `finder`, `muhurta_lagna`, `activity_profiles`, `personal_factors`, `eclipse`, `gandmool`, `kaksha`, `kantaka`, plus new `dasha_deep` and `ashtakavarga` endpoints.
- **B-16 Phase 2:** `graph_rag` + `knowledge_engine` extracted into a shared, portable pip package (`vedic_knowledge`, sibling repo `VedicAstro/vedic-knowledge/`) meant for both VedicAstro and panchanga_muhurtha to consume. VedicAstro's own `cvce/graph_rag/` and `cvce/knowledge_engine/` reduced to thin re-export shims over it. panchanga_muhurtha bridges to it via `api/vedic_knowledge_bridge.py` — HTTP call to VedicAstro's deployed CVCE in production; an in-process pip-import path exists but is local-dev-only and not in their `requirements.txt` (checked, no production coupling risk — V-12).
- **Self-evolving memory system:** new `auto_mapper.py`, `schema_mutator.py`, `session_memory.py` under `cvce/knowledge_engine/`, an ingest-watcher, and `/memory/*` API endpoints.
- **CVCE → Vercel vendoring (`04d2ffd`):** the shared `vedic_knowledge` package is vendored into `cvce/vedic_knowledge/` (kept in sync with the sibling source) so a Vercel-hosted CVCE can import it without a monorepo. Vercel project `vedicastro-cvce-vercel` (stable alias `vedicastro-cvce-vercel.vercel.app`).
- **V-1/V-13, real production incident, fixed:** the shared package imported VedicAstro-local `vedic_engine`, a non-deterministic bug (uncatchable `KeyError`, import-order-dependent) independently traced from the panchanga_muhurtha side to a real Supabase org-wide egress-quota trip that broke that app's admin dashboard. Fixed via dependency injection; an uncached-scan egress fix shipped in the same commit (`c779525`, pushed). 340/340 CVCE tests pass.
- **V-7, Fly→Vercel migration — fully cut over and verified.** Owner-approved (root cause: Muhurtha's Fly free tier was unintentionally billed and froze). Sequence: pushed pending commits → generated and synced a new shared `CVCE_SERVICE_TOKEN` across both Vercel projects (they'd never matched) → redeployed CVCE → set the portal's production `CVCE_BASE_URL` to the Vercel CVCE and redeployed → verified with a real end-to-end request through the actual production path (`portal-omega-two-10.vercel.app/api/cvce/positions` → the live Vercel CVCE), correct golden reference chart returned. Fly is left running as a fallback, not decommissioned.
- **V-18, new — a real upstream bug found during that verification, not before:** `PyJHora==4.8.7`'s own `drik.planetary_positions()` crashes on any fresh install (`AttributeError`, a dict/list mismatch bug in the third-party package itself). Never surfaced locally or on Fly because those environments carried an untracked hand-patch nobody had recorded anywhere — meaning *any* fresh CVCE deploy would have silently broken `/chart`, `/natal`, `/report/facts`, `/predict`. Fixed with a self-healing monkeypatch in `cvce/app/ephem.py`; verified in an isolated Python 3.12 venv matching Vercel's exact build, the full CVCE suite, and live authenticated production requests.
- **V-4 (B2 experiment system) and V-8 (Transit Context)** both got fresh from-scratch independent reviews (the original 66/100 and 68/100 verdicts' per-finding detail was never preserved anywhere to remediate against). Delegated the raw adversarial pass to DeepSeek, then individually verified every claim against the actual code before trusting any of it — most flagged "critical"/"high" findings for both were false positives once traced through the real invariants and dependencies. V-4: no real bugs, 30/30 tests pass. V-8: 2 small genuine issues found and fixed (unmount-safe place search, a clearer geolocation-timeout message), all tests green.
- **V-11, Life-Event Prediction engine — implementation deliberately gated, not started.** Owner's explicit decision: hold until the Vercel cutover was fully closed (it is now), and even once unblocked, a MAFIP ≥95/100 remediate-and-re-gate cycle stays mandatory regardless of BOSS/DeepSeek delegation being used to draft it. Design review done: every file:line citation in `docs/prediction-engine-strategy/LIFE_EVENT_ENGINE_PLAN.md` checked against current code, still fully accurate a month later — one real gap found (the witness pseudocode's karaka-presence check is vacuous as written, needs resolving before implementation). A golden-chart fixture was pulled live from the running engine; synthetic branch-coverage fixtures deliberately deferred to the implementation session itself, since the functions they'd test don't exist yet.
- **Other backlog items closed this session:** stale `RELEASE_HANDOFF.md` B3 text; 456/458 dormant `.kilo/worktrees/*` pruned; `graph-deepseek.json`'s lower node count confirmed as a separate diagnostic-only extraction; a Supabase RLS security fix confirmed live; 2 failed book-embedding slices re-run successfully; panchanga_muhurtha's shared-package usage confirmed HTTP-only in production.
- **Not urgent, no action needed right now:** V-16, a dormant launchd daemon's old crash history — only relevant if ingestion gets re-run.
...
|-----------|----------|---------|--------|
| CVCE (engine) | `cvce/` | Python 3.12 / FastAPI | Fly.io (`vedicastro-cvce`, region `lhr`) — being migrated to Vercel (`vedicastro-cvce-vercel`), see 2026-08-11 checkpoint above; portal still defaults to Fly until cutover completes |
