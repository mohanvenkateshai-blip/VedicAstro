...
class DashaResult:
    ...
    current_mahadasha: DashaPeriod | None = None
    current_antardasha: DashaPeriod | None = None
    all_mahadashas: list = field(default_factory=list)

    # Yogini
    balance_of_yogini: float = 0
    yogini_start: YoginiDasha | None = None
    current_yogini: YoginiDasha | None = None
    current_yogini_antardasha: DashaPeriod | None = None
...
def compute_vimshottari(
    ...
    for i in range(1, 9):
        ...
        dasha_periods.append(
            {"planet": lord, "start": current, "end": end, "years": years, "level": "Maha"}
        )
        current = end
